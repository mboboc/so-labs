...
{
	for (i = 0; i < size; i++)
		/* fill in the correct parameters to be printed */
...
{
	for (p = ebp; p >= esp; p--) {
		stack[size].address = p;/* set the current address */
		stack[size].value   = *p;/* set the value at address */
		size++;
	}
}
...
void f2(mytype a)
{
	...
	/* use v so that when function 'f2' returns,
